...
class Trader
{
   ...
   * @return array
   */
  public static function get_args_from_request_params() : array
  {
    $defaults = array(
      'top_count'   => 30,
      'smoothing'   => 14,
   ...
   *
   * @param \Trader\Balance $balance          Existing balance.
   * @param \Trader\Balance $balance_exchange Updated balance.
   * @param array           $args {.
   ...
   */
  public static function merge_balance( $balance, $balance_exchange = null, array $args = array() ) : \Trader\Balance
  {
   ...
   */
  protected static function set_asset_allocations( $weighting, \Trader\Asset $asset, $market_cap, int $sqrt = 5 )
  {
   ...
   */
  public static function get_asset_allocations( array $assets_weightings = array(), array $args = array() )
  {
    $args = wp_parse_args(
      $args,
      array(
    ...
    foreach ( $cmc_latest as $asset_cmc_arr ) {
       ...
       * Get Market Cap EMA.
       */
      self::retrieve_market_cap_ema( $asset_cmc_arr, $market_cap_ema, $args['smoothing'] );
    ...
    $cmc_latest = array_slice( $cmc_latest, 0, $args['top_count'] );
    ...
    foreach ( $cmc_latest as $asset_cmc_arr ) {
       ...
       */
      // retrieve_allocation_indicators( $asset_cmc_arr[0], $candles, $args['interval_days'] );
   ...
   */
  public static function rebalance( \Trader\Balance $balance, string $mode = 'default', array $args = array(), bool $simulate = false ) : array
  {
    $args = wp_parse_args(
      $args,
      array(
